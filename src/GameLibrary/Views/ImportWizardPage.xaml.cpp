...
namespace winrt::GameLibrary::implementation
{
    ...
    void ImportWizardPage::ApplyLanguage()
    {
        ...
        RefreshSourceHint();
        UpdateCount();
        // 未激活提示：没有激活时向导本来就不可用，这条状态就是它的主文案，得跟着语言走
        if (!Licensing::LicenseManager::IsActivated())
        {
            StatusText().Text(hstring(loc.T(L"import.statusNeedActivation")));
        }
    }
    ...
    winrt::Windows::Foundation::IAsyncAction ImportWizardPage::ImportButton_Click(IInspectable const&, RoutedEventArgs const&)
    {
        ...
        if (!Licensing::LicenseManager::IsActivated())
        {
            auto& loc = Services::Localization::Instance();
            ContentDialog dlg;
            dlg.Title(box_value(hstring(loc.T(L"import.needActivationTitle"))));
            dlg.Content(box_value(hstring(loc.T(L"import.needActivationBody"))));
            dlg.CloseButtonText(hstring(loc.T(L"common.ok")));
