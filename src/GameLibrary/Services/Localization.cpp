    ...
    namespace
    {
        ...
        std::unordered_map<std::wstring, std::wstring> const& Zh()
        {
            static std::unordered_map<std::wstring, std::wstring> const s{
                ...
                {L"settings.rememberWindow", L"记住窗口大小"},
                {L"settings.rememberWindowHint", L"开启时恢复上次退出时的窗口位置与大小；关闭后每次启动都居中显示。"},
        ...
        std::unordered_map<std::wstring, std::wstring> const& En()
        {
            static std::unordered_map<std::wstring, std::wstring> const s{
                ...
                {L"settings.rememberWindow", L"Remember window size"},
                {L"settings.rememberWindowHint", L"Restores the window position and size from your last session; when off, the window is centered on every launch."},
